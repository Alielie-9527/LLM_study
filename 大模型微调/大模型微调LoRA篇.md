...
​	LoRA的初始化中，一个比较重要的就是低秩矩阵A和B的初始化方式，会对结果带来不同的影响。影响训练稳定性、收敛速度，并最终影响微调模型的质量。

### 默认初始化策略：B为零，A为高斯初始化

1. **初始化矩阵A**：用标准的随机初始化方式，比如**Kaiming均匀**或者**高斯分布**$\mathcal{N}(0, \sigma^2)$。
2. **初始化矩阵B**：全零

​	这种方式，在微调刚开始的时候，保证了$\Delta W = 0$，也就是**模型刚开始微调时的行为与基础模型是一致的**，然后反向传播改变了B的时候,$\Delta W$才开始调整。

**优点**：

- **稳定性**：开始训练时，没有对预训练成果造成干扰。一般来说训练初期更加稳定（**平滑过渡**）。
- **保留预训练知识**

**缺点**：

- **初始学习较慢**：B矩阵一开始为0，这时候A的梯度是0，所以需要一段时间的更新才能产生有意义的适应。

### A和B均采用高斯初始化（次优）

​	选择合适的方差$\sigma^2$，对A和B采取高斯初始化$\mathcal{N}(0, \sigma^2)$，$\sigma$越大，意味着数据越分散（相距0越远）。也就意味着，初始的随机化$\Delta W$提供了一个**可能**带有自适应方向的噪声，希望让模型更快调整。

**注意**：

- **方差大小需要合理选取**：过小，那么数值都在0附近，更新慢，作用微不足道;过大，则会导致显著干扰模型输出，导致训练不稳定或者性能下降。
- 不稳定性以及对参数的敏感性增加（学习率、$\sigma$）
