...
3.  **循环迭代**（直到所有权重都被量化）：
    ...
    *   **选择**：找到 Error 最小的那个权重 $w_{\text{best}}$。
    *   **量化**：将 $w_{\text{best}}$ 量化，算出误差 $\Delta = q - w_{\text{best}}$。
    *   **补偿（核心步骤）**：利用 OBQ 更新公式，把 $\Delta$ 通过 $H^{-1}$ 的相关性，"抹平"到剩余所有未量化的权重上。
        *   注意：此时剩余权重的数值变了！这意味着原本可能量化误差很大的数，被补偿后可能变得容易量化了。
    *   **更新 Hessian**：因为 $w_{\text{best}}$ 已经固定了，它不再是变量，我们需要从 Hessian 矩阵中移除这一行一列（高斯消元）想具体了解可以参考：[GPTQ大模型量化算法原理详解（一） - 知乎](https://zhuanlan.zhihu.com/p/1929142220717537176)。
...
*   每次处理一个权重，都要更新整个权重矩阵。
*   每次都要在所有剩余权重里搜索下一个。
*   复杂度是 $O(d_{\text{row}} \cdot d_{\text{col}}^2)$。对于大模型（如 Llama 70B），这需要跑几百个小时，根本不可用。
...
2.  **Cholesky 分解：**
    ...
    $$
    H^{-1}_{\text{new}} = H^{-1}_{\text{sub}} - \frac{\mathbf{v}\mathbf{v}^T}{h_{11}}
    $$
    ...
    $$
    H^{-1} = \begin{bmatrix} a_{11} & \mathbf{v}^T \\ \mathbf{v} & H^{-1}_{\text{sub}} \end{bmatrix} 
    $$
    *   $a_{11}$：左上角的元素。
    *   $\mathbf{v}$：第一列剩下的部分。
    *   $H^{-1}_{\text{sub}}$：右下角的子矩阵。

    同样，我们将目标矩阵 $L$ 也切分：
    $$
    L = \begin{bmatrix} l_{11} & 0 \\ \mathbf{l} & L_{\text{next}} \end{bmatrix}
    $$

    *   $l_{11}$：左上角元素。
    *   $\mathbf{l}$：第 1 列剩下的部分。
    *   $L_{\text{next}}$：右下角的子下三角矩阵（我们要递归求的部分）。

    建立等式：$A = L L^T$
    $$
    L L^T = \begin{bmatrix} l_{11} & 0 \\ \mathbf{l} & L_{\text{next}} \end{bmatrix} \times \begin{bmatrix} l_{11} & \mathbf{l}^T \\ 0 & L_{\text{next}}^T \end{bmatrix} = \begin{bmatrix} a_{11} & \mathbf{v}^T \\ \mathbf{v} & H^{-1}_{\text{sub}} \end{bmatrix}
    $$
    ...
    （3）$$ \mathbf{H^{-1}_{\text{sub}}} = \mathbf{l}\mathbf{l}^T + L_{\text{next}}L_{\text{next}}^T $$ **最关键的一步**

    我们要解出 $L_{\text{next}}$（为了继续递归），需要把 $\mathbf{l}\mathbf{l}^T$ 移项到左边：
    $$
    \underbrace{L_{\text{next}}L_{\text{next}}^T}_{\text{下一步的目标}} = \underbrace{\mathbf{H^{-1}_{\text{sub}}} - \mathbf{l}\mathbf{l}^T}_{\text{这就是 } \mathbf{H^{-1}_{\text{new}}}}
    $$
    然后带入 $\mathbf{l}$:
    $$
    \underbrace{H^{-1}_{\text{new}}}_{\text{更新后的逆矩阵}} 
    = \underbrace{H^{-1}_{\text{sub}}}_{\text{右下角子矩阵}} - \underbrace{\mathbf{l}\mathbf{l}^T}_{\text{Cholesky 减去的项}} 
    = \underbrace{H^{-1}_{\text{sub}} - \frac{\mathbf{v}\mathbf{v}^T}{h_{11}}}_{\text{OBQ/高斯消元 减去的项}}
    $$
    如此迭代下去，所有的 $H$ 就提前算好了。
...
3.  **Lazy Batch（懒惰批量）策略**
    ...
    **步骤演示：**
    ...
    2.  **全局更新（GEMM 大招）：**
        ...
        *   **$W_{\text{rest}} \leftarrow W_{\text{rest}} - \text{Block\_Coeffs} \times \text{Block\_Errors}$**
